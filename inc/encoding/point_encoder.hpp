...
public:
  ...
  virtual void encodeVectorized(const uint32_t *x, const uint32_t *y,
                                const uint32_t *z, std::vector<key_t> &keys,
                                size_t i) const = 0;
  virtual void decode(key_t code, coords_t &x, coords_t &y,
                      coords_t &z) const = 0;
  ...
  inline void getAnchorCoords(const Point &p, const Box &bbox, coords_t &x,
                              coords_t &y, coords_t &z) const {
    // Put physical coords into the unit cube
    double x_transf =
        ((p.getX() - bbox.center().getX()) + bbox.radii().getX()) /
        (2 * bbox.radii().getX());
    double y_transf =
        ((p.getY() - bbox.center().getY()) + bbox.radii().getY()) /
        (2 * bbox.radii().getY());
    double z_transf =
        ((p.getZ() - bbox.center().getZ()) + bbox.radii().getZ()) /
        (2 * bbox.radii().getZ());

    // Scale to [0,2^L)^3 for morton encoding, handle edge case where coordinate
    // could be 2^L if _transf is exactly 1.0
  ...
  /// @brief A wrapper for doing PointEncoding::getAnchorCoords + encode in one
  /// single step
  virtual key_t encodeFromPoint(const Point &p, const Box &bbox) const = 0;

  template <PointContainer Container>
  std::vector<key_t> encodePoints(const Container &points,
...
#pragma omp parallel for schedule(static)
  ...
  template <PointContainer Container>
  void encodePointsVectorized(const Container &points, const Box &bbox,
                              std::vector<key_t> &keys) const {
    size_t n = points.size();

    if constexpr (std::is_same_v<Container,
                                 PointsSoA>) // Only to avoid compilation errors
    {
...
#pragma omp for schedule(static)
        for (size_t i = 0; i < n - 15; i += 16) {
          ...
          // Put physical coords into the unit cube: ((p - center) + radii) / (2
          // * radii)
          ...
          // Scale to [0,2^L)^3 for morton encoding, handle edge case where
          // coordinate could be 2^L if _transf is exactly 1.0
          __m512d x_scaled =
              _mm512_min_pd(_mm512_mul_pd(x_transf, scale), maxCoord);
          __m512d y_scaled =
              _mm512_min_pd(_mm512_mul_pd(y_transf, scale), maxCoord);
          __m512d z_scaled =
              _mm512_min_pd(_mm512_mul_pd(z_transf, scale), maxCoord);
...
#pragma omp for schedule(static)
        for (size_t i = 0; i < n - 7; i += 8) {
          ...
          // Put physical coords into the unit cube: ((p - center) + radii) / (2
          // * radii)
          __m256d x_transf = _mm256_div_pd(
              _mm256_add_pd(_mm256_sub_pd(pointsX, bboxCenterX), bboxRadiiX),
              _mm256_mul_pd(two, bboxRadiiX));
          __m256d y_transf = _mm256_div_pd(
              _mm256_add_pd(_mm256_sub_pd(pointsY, bboxCenterY), bboxRadiiY),
              _mm256_mul_pd(two, bboxRadiiY));
          __m256d z_transf = _mm256_div_pd(
              _mm256_add_pd(_mm256_sub_pd(pointsZ, bboxCenterZ), bboxRadiiZ),
              _mm256_mul_pd(two, bboxRadiiZ));

          // Scale to [0,2^L)^3 for morton encoding, handle edge case where
          // coordinate could be 2^L if _transf is exactly 1.0
          __m256d x_scaled =
              _mm256_min_pd(_mm256_mul_pd(x_transf, scale), maxCoord);
          __m256d y_scaled =
              _mm256_min_pd(_mm256_mul_pd(y_transf, scale), maxCoord);
          __m256d z_scaled =
              _mm256_min_pd(_mm256_mul_pd(z_transf, scale), maxCoord);
          ...
          // Put physical coords into the unit cube: ((p - center) + radii) / (2
          // * radii)
          x_transf = _mm256_div_pd(
              _mm256_add_pd(_mm256_sub_pd(pointsX, bboxCenterX), bboxRadiiX),
              _mm256_mul_pd(two, bboxRadiiX));
          y_transf = _mm256_div_pd(
              _mm256_add_pd(_mm256_sub_pd(pointsY, bboxCenterY), bboxRadiiY),
              _mm256_mul_pd(two, bboxRadiiY));
          z_transf = _mm256_div_pd(
              _mm256_add_pd(_mm256_sub_pd(pointsZ, bboxCenterZ), bboxRadiiZ),
              _mm256_mul_pd(two, bboxRadiiZ));
...
#pragma omp single nowait
  ...
  std::vector<key_t> sortPoints(
      Container &points, std::optional<std::vector<PointMetadata>> &meta_opt,
      const Box &bbox, std::shared_ptr<EncodingLog> log = nullptr) const {
    size_t n = points.size();
    ...
    // Encoding
    if constexpr (std::is_same_v<std::decay_t<Container>, PointsSoA>) {
      if (this->getEncoder() == EncoderType::HILBERT_ENCODER_3D) {
        keys.resize(n);
    ...
    for (int pass = 0; pass < NUM_PASSES; pass++) {
      ...
      std::vector<std::vector<size_t>> localHist(
          nThreads, std::vector<size_t>(NUM_BUCKETS, 0));
...
#pragma omp for schedule(static)
   ...
   */
  template <PointContainer Container>
  std::vector<key_t> sortPointsOriginal(
      Container &points, std::optional<std::vector<PointMetadata>> &meta_opt,
      const Box &bbox, std::shared_ptr<EncodingLog> log = nullptr) const {
    size_t n = points.size();
    // Choose 4, 8 or 16. 16 is less passes but requires more memory for the
    // histograms and it offers better performance in most cases.
...
#pragma omp parallel for schedule(static)
   ...
   *
   * @details This is the main reordering function that we use to achieve
   * spatial locality during neighbourhood searches. Encodings are first
   * computed using encodeFromPoint, put into a pairs vector and then reordered.
   *
   * @param points The input/output array of 3D points (i.e. the point cloud)
   * @param codes The output array of computed codes (allocated here, only pass
   * unallocated reference)
   * @param bbox The precomputed global bounding box of points
   * @param meta_opt The optional metadata of the point cloud (if Lpoint is
   * used, it is contained on the struct). meta_opt is a parallel array to
   * points and will be sorted in the same order
   */
  template <PointContainer Container>
  std::pair<std::vector<key_t>, Box>
  sortPoints(Container &points,
    ...
    return std::make_pair(sortPoints<Container>(points, meta_opt, bbox, log),
                          bbox);
  }
  ...
  inline Point getCenter(key_t code, uint32_t level, const Box &bbox,
                         ...
                         const std::vector<Vector> &precomputedRadii) const {
    // Decode the points back into their integer coordinates
    coords_t min_x, min_y, min_z;
    decode(code, min_x, min_y, min_z);

    // Now adjust the coordinates so they indicate the lowest code in the
    // current level In Morton curves this is not needed, but in Hilbert curves
    // it is, since it can return any corner instead of lower one we need Fun
    // fact: finding this mistake when adding Hilbert curves took 6 hours of
    // debugging
    coords_t mask =
        ((1u << maxDepth()) - 1) ^ ((1u << (maxDepth() - level)) - 1);
    min_x &= mask, min_y &= mask, min_z &= mask;

    // Find the physical center by multiplying the encoding with the halfLength
    // to get to the low corner of the cell, and then adding the radii of the
    // cell
    Point center = Point(bbox.minX() + min_x * halfLengths[0] * 2,
   ...
   *
   * @example At level 0 the range is the entire 63 bit span, at level 10 the
   * range is 11*3 bit span at level 20 (last to minimum), the range will just
   * be 8 between each node, i.e. the 8 siblings that can be on max level 21
   * between two nodes at level 20
   *
  ...
  /**
   * @brief Transforms the SFC (leaf) format into the placeholder bit format, by
   * putting non empty octants into the beginning of the key and then adding a
   * placeholder bit after them.
   * @example 0 100 000 000 000 ... 000 --> 0 000 000 ... 000 001 100
   * @tparam Encoder The encoder type.
   * @param code The encoded key.
   * @param level The level of the octree on which we are, i.e. the number of
   * octants considered in the key
  ...
  /// @brief Extract the octal digit at a given position in a code in the SFC
  /// format.
