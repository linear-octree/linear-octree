...
template <PointContainer Container> class Octree {
private:
...
public:
  ...
  Octree(Container &container, Point center, Vector radii,
         std::vector<size_t> &points, const size_t maxPointsLeaf = 128);

  [[nodiscard]] inline const std::vector<Octree> &getOctants() const {
    return octants_;
  }

  inline void setCenter(const Point &center) { center_ = center; }

  [[nodiscard]] inline const std::vector<size_t> &getPoints() const {
    return points_;
  }
   ...
   */
  {
    return static_cast<double>(points_.size()) /
           (radii_.getX() * radii_.getY() + radii_.getZ());
  }
  ...
  [[nodiscard]] inline std::vector<size_t> searchNeighbors(const Point &p,
                                                           double radius) const
  /**
   * @brief Search neighbors function. Given a point and a radius, return the
   * points inside a given kernel type
   ...
   */
  {
    ...
    // Dummy condition that always returns true, so we can use the same function
    // for all cases The compiler should optimize this away
  ...
  searchNeighbors(const Point &p, const Vector &radii) const
  /**
   * @brief Search neighbors function. Given a point and a radius, return the
   * points inside a given kernel type
   * @param p Center of the kernel to be used
  ...
  [[nodiscard]] inline std::vector<size_t>
  searchNeighbors(const Point &p, double radius, Function &&condition) const
  /**
   * @brief Search neighbors function. Given a point and a radius, return the
   * points inside a given kernel type
   * @param p Center of the kernel to be used
   ...
   * function should be equivalent to `bool cnd(const Point &p);`
   * @return Points inside the given kernel type
   */
  {
    const auto kernel = kernelFactory<kernel_type>(p, radius);
  ...
  template <Kernel_t kernel_type = Kernel_t::square, class Function>
  [[nodiscard]] inline std::vector<size_t>
  searchNeighbors(const Point &p, const Vector &radii,
  ...
  [[nodiscard]] std::vector<size_t> KNN(const Point &p, size_t k,
                                        size_t maxNeighs = DEFAULT_KNN) const;
   ...
   * point and a radius, return the points inside the region.
   * @param k specific kernel that contains the data of the region (center and
   * radius)
   * @param condition function that takes a candidate neighbor point and imposes
   * an additional condition (should return a boolean). The signature of the
   * function should be equivalent to `bool cnd(const Point &p);`
   * @return Points inside the given kernel type. Actually the same as
   * ptsInside.
   */
  {
    ...
    toVisit.reserve(
        OCTANTS_PER_NODE); // There is usually less than 8 octants to visit
    ...
    while (!toVisit.empty()) {
      ...
      if (octree.isLeaf()) {
        ...
        std::copy_if(std::begin(octree.octants_), std::end(octree.octants_),
                     std::back_inserter(toVisit), [&](const Octree &octant) {
                       return k.boxOverlap(octant.getCenter(),
                                           octant.getRadii());
                     });
      }
    }
  ...
  [[nodiscard]] inline std::vector<size_t>
  searchNeighbors2D(const Point &p, const double radius) const {
    return searchNeighbors<Kernel_t::square>(p, radius);
  }

  [[nodiscard]] inline std::vector<size_t>
  searchCylinderNeighbors(const Point &p, const double radius,
                          const double zMin, const double zMax) const {
    return searchNeighbors<Kernel_t::circle>(p, radius, [&](const Point &p) {
      return p.getZ() >= zMin && p.getZ() <= zMax;
    });
  }

  [[nodiscard]] inline std::vector<size_t>
  searchCircleNeighbors(const Point &p, const double radius) const {
  ...
  [[nodiscard]] inline std::vector<size_t>
  searchNeighbors3D(const Point &p, const Vector &radii) const {
    return searchNeighbors<Kernel_t::cube>(p, radii);
  }

  [[nodiscard]] inline std::vector<size_t>
  searchNeighbors3D(const Point &p, double radius) const {
    return searchNeighbors<Kernel_t::cube>(p, radius);
  }

  [[nodiscard]] inline std::vector<size_t>
  searchNeighbors3D(const Point &p, const double radius,
   ...
   * @param flags Vector of flags: return only points which flags[pointId] ==
   * false
   * @return Points inside the given kernel
   */
  {
    const auto condition = [&](const Point &point) {
      return !flags[point.id()];
    };
    return searchNeighbors<Kernel_t::cube>(p, radius, condition);
  }

  template <Kernel_t kernel_type = Kernel_t::square>
  [[nodiscard]] inline size_t numNeighbors(const Point &p,
                                           const double radius) const
  /**
   * @brief Search neighbors function. Given a point and a radius, return the
   * number of points inside a given kernel type
   * @param p Center of the kernel to be used
   * @param radius Radius of the kernel to be used
   * @param condition function that takes a candidate neighbor point and imposes
   * an additional condition (should return a boolean). The signature of the
  ...
  template <Kernel_t kernel_type = Kernel_t::square, class Function>
  [[nodiscard]] inline size_t numNeighbors(const Point &p, const double radius,
                                           Function &&condition) const
  /**
   * @brief Search neighbors function. Given a point and a radius, return the
   * number of points inside a given kernel type
  ...
  template <typename Kernel, typename Function>
  [[nodiscard]] size_t numNeighbors(const Kernel &k,
                                    Function &&condition) const {
    size_t ptsInside = 0;
    ...
    while (!toVisit.empty()) {
      ...
      if (octree.isLeaf()) {
        for (size_t globalIdx : octree.getPoints()) {
          const auto &point = octree.container_[globalIdx];
          if (k.isInside(point) && k.center().id() != point.id() &&
              condition(point)) {
            ++ptsInside;
          }
        }
  ...
  /** Inside a sphere */
  [[nodiscard]] inline std::vector<size_t>
  searchSphereNeighbors(const Point &point, const float radius) const {
    return searchNeighbors<Kernel_t::sphere>(point, radius);
  }

  [[nodiscard]] std::vector<size_t>
  searchNeighborsRing(const Point &p, const Vector &innerRingRadii,
                      const Vector &outerRingRadii) const
  /**
   * A point is considered to be inside a Ring around a point if its outside the
   * innerRing and inside the outerRing
   * @param p Center of the kernel to be used
   * @param innerRingRadii Radii of the inner part of the ring. Points within
   ...
   */
  {
    ...
    const auto condition = [&](const Point &point) {
      return !innerKernel.isInside(point);
    };
  ...
  // std::vector<Point_t*> searchEraseCircleNeighbors(const std::vector<Point>&
  // points, double radius);

  // /** Inside a sphere */
  // std::vector<Point_t*> searchEraseSphereNeighbors(const std::vector<Point>&
  // points, float radius);

  // /** Connected inside a spherical shell*/
  // [[nodiscard]] std::vector<Point_t*> searchConnectedShellNeighbors(const
  // Point& point, float nextDoorDistance,
  //                                                                  float
  //                                                                  minRadius,
  //                                                                  float
  //                                                                  maxRadius)
  ...
  // std::vector<Point_t*> searchEraseConnectedCircleNeighbors(float
  // nextDoorDistance);

  // static std::vector<Point_t*> connectedNeighbors(const Point* point,
  // std::vector<Point_t*>& neighbors,
  //                                                float nextDoorDistance);

  // static std::vector<Point_t*> extractCloseNeighbors(const Point* p,
  // std::vector<Point_t*>& neighbors, float radius);

  // std::vector<Point_t*> kClosestCircleNeighbors(const Point_t* p, size_t k)
  // const; std::vector<Point_t*> nCircleNeighbors(const Point_t* p, size_t n,
  // float& radius, float minRadius, float maxRadius,
  //                                       float maxIncrement = 0.25, float
  //                                       maxDecrement = 0.25) const;

  // std::vector<Point_t*> nSphereNeighbors(const Point_t& p, size_t n, float&
  // radius, float minRadius, float maxRadius,
