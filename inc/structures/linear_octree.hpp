 ...
 * @class LinearOctree
 *
 * @details This linear octree is built by storing offsets to the positions of
 * an array of points sorted by their morton codes. For each leaf of the octree,
 * there is an element in this array listthat points to the index of the first
 * point in that leaf. Since the points are sorted, the next element on the
 * array - 1 contains the index of the last point in that leaf.
 *
 * @cite Keller et al. Cornerstone: Octree Construction Algorithms for Scalable
 * Particle Simulations. https://arxiv.org/pdf/2307.06345
 *
...
template <PointContainer Container> class LinearOctree {
...
protected:
  ...
  /// @brief The minimum octant radius to have in a leaf (TODO: this could be
  /// implemented in halfLength compuutation, but do we really need it?)
  ...
  struct LeafPart {
     ...
     * @details This array contains encoded (Hilbert or Morton) points
     * (interpreted here as octal digit numbers) satisfying certain constraints:
     * 1. The length of the array is nLeaf + 1
     * 2. The first element is 0 and the last element 8^MAX_DEPTH, where
     * MAX_DEPTH is the maximum depth of the encoding system (i.e. an upper
     * bound for every possible encoding of a point)
     * 3. The array is sorted in increasing order and the distance between two
     * consecutive elements is 8^l, where l is less or equal to MAX_DEPTH
     *
     * The array is initialized to {0, 8^MAX_DEPTH} and then subdivided into 8
     * equally sized bins if the number of points with encoding between two
     * leaves is greater than mainOptions.maxPointsLeaf.
     *
     * For more details about the construction, check the cornerstone paper,
     * section 4.
     */
    std::vector<key_t> leaves;

    /// @brief  This array contains how many points have an encoding with a
    /// value between two of the leaves
    std::vector<size_t> counts;

    /// @brief This array is simply an exclusive scan of counts, and marks the
    /// index of the first point for a leaf
  ...
  struct InternalPart {
     ...
     * @details For a given (internal or leaf) node, we store its position on
     * the octree using this array, the position for a node at depth n will be
     * given by 0 000 000 ... 1 x1y1z1 ... xnynzn. This allows for traversals
     * needed in neighbourhood search.
     *
     * The process to obtain this array and link it with the leaves array is
     * detailed in the cornerstone paper, section 5.
     */
    ...
    /// @brief First node index of every tree level (L+2 elements where L is
    /// MAX_DEPTH)
    std::vector<size_t> levelRange;

    /// @brief A map between the internal representation at offsets and the one
    /// in cornerstone format in leaves
  ...
  /// @brief Number of leaves and internal nodes in the octree. Equal to size of
  /// the leaves vector - 1.
  uint32_t nLeaf;
  ...
  /// @brief The maximum number of points that a leaf can have before being
  /// subdivided, this is the main parameter that controls the shape of the
  /// octree
   ...
   */
  /// @brief Index of the first child of each node (if 0 we have a leaf). This
  /// is the array used during DFS Octree searches.
  std::vector<uint32_t> offsets;

  /// @brief This array is built from exclusiveScan via a traversal, and marks
  /// the index of the first and last points for a leaf or internal node.
  std::vector<std::pair<size_t, size_t>> internalRanges;

  /// @brief The center points of each node in the octree
  std::vector<Point> centers;

  /// @brief A simple vector containinf the radii of each level in the octree to
  /// speed up computations.
  std::vector<Vector> precomputedRadii;

  /**
   * @brief A reference to the array of points that we sort
   * @details At the beginning of the octree construction, this points are
   * encoded and then sorted in-place in the order given by their encodings.
   * Therefore, this array is altered inside this class. This is done to
   * locality that Morton/Hilbert
   */
  Container &points;
  ...
  /// @brief A reference to the PointEncoder used to sort the cloud before
  /// starting the LinearOctree build.
  PointEncoder &enc;

  /// @brief The encodings of the points in the octree, not needed during
  /// searches so we don't count towards total memory overhead
  std::vector<key_t> &codes;

  /// @brief A vector containing the half-lengths of the minimum measure of the
  /// encoding.
  double halfLengths[3];

  /**
   * @brief Computes the rebalacing decisions as the first process in the
   * subdivision of the leaves array
   *
   * @details This function implements g1 in the cornerstone paper, for each
   * leaf we calculate the operation that decides whether we merge, split or
   * leave unchanged the leaf.
   *
...
#pragma omp parallel for schedule(static)
   ...
   * - 0 if the leaf should be merged (only if the counts of all of its siblings
   * are <= maxPointsLeaf_), the siblings are all next to each other and the
   * node is not the first sibling (because the first sibling is the node that
   * will stay after merge)
   * - 8^L where L goes up to 4, if we need to split the node L times
   * (recursively)
   *
  ...
  uint32_t calculateNodeOp(LeafPart &leaf, uint32_t index) {
    ...
    if (sibling > 0) {
      // We have 8 siblings next to each other, could merge this node if the
      // count of all siblings is less MAX_COUNT
      uint32_t parentIndex = index - sibling;
      size_t parentCount =
          leaf.counts[parentIndex] + leaf.counts[parentIndex + 1] +
   ...
   *
   * @details Will return -1 for the sibling if the nodes are not next to each
   * other or if the level is 0.
   *
   * @param index The leaf index to find
   */
  inline std::pair<int32_t, uint32_t> siblingAndLevel(LeafPart &leaf,
                                                      uint32_t index) {
    ...
    // Checks if all siblings are on the tree, to do this, checks if the
    // difference between the two parent nodes corresponding to the code parent
    // and the next parent is the range spanned by two consecutive codes at that
    // level
    bool siblingsOnTree =
        leaf.leaves[index - siblingId + 8] ==
        (leaf.leaves[index - siblingId] + enc.nodeRange(level - 1));
    if (!siblingsOnTree)
      siblingId = -1;
  ...
  /**
   * @brief Computes a new stage of the leaves array by subdividing using the
   * operations given
   *
   * @details This function implements steps g2 and g3 of the subdivision
   * process
   *
   * @param newLeaves The new leaves array that will be swapped with the current
   * one after this function execution
   * @param nodeOps The array of operations performed in the first step (g1)
   */
  void rebalanceTree(LeafPart &leaf, std::vector<key_t> &newLeaves,
                     std::vector<size_t> &nodeOps) {
...
#pragma omp parallel for schedule(static)
   ...
   *
   * @details Sometimes more than one index is constructed, when nodeOps = 8 or
   * higher, the values are put for all the new siblings that the leaves array
   * subdivides into. This function implements step g3 for each element.
   *
   ...
   * one after this function execution
   */
  void processNode(LeafPart &leaf, size_t index, std::vector<size_t> &nodeOps,
    ...
    // The new position to put the node into (nodeOps value after exclusive
    // scan)
    ...
    if (opCode > 1) {
      // Split the node into 8^L as marked by the opCode, add the adequate codes
      // to the new leaves
      uint32_t levelDiff = enc.log8ceil(opCode);
      key_t gap = enc.nodeRange(level + levelDiff);
      for (size_t sibling = 1; sibling < opCode; sibling++) {
        newLeaves[newNodeIndex + sibling] =
            newLeaves[newNodeIndex + sibling - 1] + gap;
      }
    }
  }
   ...
   *
   * @details This functions counts how many particles have encodings at leaf i,
   * that is between leaves[i] and leaves[i+1]
   */
  void computeNodeCounts(LeafPart &leaf) {
    size_t n = 0;
    if (leaf.leaves.size())
      n = leaf.leaves.size() - 1;
    ...
    if (codes.size() > 0) {
      firstNode =
          std::upper_bound(leaf.leaves.begin(), leaf.leaves.end(), codes[0]) -
          leaf.leaves.begin() - 1;
      lastNode = std::upper_bound(leaf.leaves.begin(), leaf.leaves.end(),
                                  codes[codes_size - 1]) -
                 leaf.leaves.begin();
    } else {
...
// Fill non-populated parts of the octree with zeros and the inside with
// calculateNodeCount
#pragma omp parallel for schedule(static)
    for (size_t i = 0; i < n; i++) {
  ...
  /// @brief Since the codes array is sorted, we can use binary search to
  /// accelerate the counts computation
  ...
  template <class Time_t> void exclusiveScan(Time_t *out, size_t numElements) {
    Time_t a = Time_t(0);
    Time_t b = Time_t(0);
  ...
  /// @brief Computes the key weight mapping for conversion between internal and
  /// leaf nodes
  ...
  void createUnsortedLayout(LeafPart &leaf, InternalPart &inter) {
    for (size_t i = 0; i < nLeaf; i++) {
      ...
      if (prefixLength % 3 == 0 && i < nLeaf - 1) {
        ...
        inter.prefixes[octIndex] =
            enc.encodePlaceholderBit(key, prefixLength / 3);
  ...
  void getLevelRange(InternalPart &inter) {
    for (uint32_t level = 0; level <= enc.maxDepth(); level++) {
      auto it = std::lower_bound(inter.prefixes.begin(), inter.prefixes.end(),
                                 enc.encodePlaceholderBit(0, level));
      inter.levelRange[level] = std::distance(inter.prefixes.begin(), it);
    }
    inter.levelRange[enc.maxDepth() + 1] = nTotal;
  }

  /// @brief Extract parent/child relationships from binary tree and translate
  /// to sorted order
...
#pragma omp parallel for schedule(static)
    for (int i = 0; i < nInternal; i++) {
      ...
      auto childIdx = std::distance(
          inter.prefixes.begin(),
          std::lower_bound(inter.prefixes.begin() + leafSearchStart,
                           inter.prefixes.begin() + leafSearchEnd,
                           childPrefix));

      if (childIdx != leafSearchEnd &&
          childPrefix == inter.prefixes[childIdx]) {
  ...
  /// @brief Computes the ranges of point indexes covered by internal or leafs
  /// nodes
  std::pair<size_t, size_t> computeInternalRanges(LeafPart &leaf,
                                                  InternalPart &inter,
                                                  uint32_t node = 0) {
    // If node is a leaf, get its internal layout from the two consecutive leafs
    // on the layout array
    if (offsets[node] == 0) {
      internalRanges[node] =
          std::make_pair(leaf.layout[inter.internalToLeaf[node]],
                         leaf.layout[inter.internalToLeaf[node] + 1]);
      size_t range_size =
          internalRanges[node].second - internalRanges[node].first;
      return internalRanges[node];
    }

    // Compute recursively (post-order DFS) the count of the internal node. It
    // will be the total range spanned by its children.
   ...
   * 2. Compute the half-lengths vector that indicates how much we displace in
   * the physical step for each unit of the morton encoded integer coordinates
  ...
  void setupBbox(InternalPart &inter) {
    ...
    // Compute the physical half lengths for multiplying with the encoded
    // coordinates
    ...
    for (int i = 0; i <= enc.maxDepth(); i++) {
      ...
      precomputedRadii[i] =
          Vector(sideLength * halfLengths[0], sideLength * halfLengths[1],
                 sideLength * halfLengths[2]);
    }
  }

  /// @brief Builds the octeee leaves array by repeatingly calling @ref
  /// updateOctreeLeaves()
  void buildOctreeLeaves(LeafPart &leaf) {
    // Builds the octree sequentially using the cornerstone algorithm
    // We start with 0, UPPER_BOUND on the leaves. Remember that UPPER_BOUND is
    // 100000...000 with as many 0s as MAX_DEPTH, and it can never be reached by
    // a code
    ...
    // Perform the exclusive scan to get the layout indices (first index in the
    // codes for each leaf)
    leaf.layout.resize(nLeaf + 1);
    std::exclusive_scan(leaf.counts.begin(), leaf.counts.end() + 1,
                        leaf.layout.begin(), 0);
  }

  /**
   * @brief Computes the node operations to be done on the leaves and modifies
   * the tree if necessary
   *
   * @details Convergence is achieved when all the node operations to be done
   * are equal to 1
   */
   ...
   *
   * @details Follows the process indicated in the cornerstone octree paper,
   * section 5.
   */
  void buildOctreeInternal(LeafPart &leaf, InternalPart &inter) {
    createUnsortedLayout(leaf, inter);
    // Sort by key where the keys are the prefixes and the values to sort
    // internalToLeaf
    ...
    std::stable_sort(
        prefixes_internalToLeaf.begin(), prefixes_internalToLeaf.end(),
  ...
  /**
   * @brief Computes the octree geometry (the centers and radii of each internal
   * node and leaf)
   *
   * @details We do this to allow for faster traversals, however this is not
   * strictly necessary and could be removed if memory becomes a constraint.
   * This stuff can be computed on the fly during neighbourhood searches.
   */
...
#pragma omp parallel for schedule(static)
    for (uint32_t i = 0; i < inter.prefixes.size(); i++) {
      ...
      centers[i] =
          enc.getCenter(startKey, level, box, halfLengths, precomputedRadii);
    }
  }

  void buildOctree(std::shared_ptr<BuildLog> log = nullptr) {
    // Temporal structs for the variables in the leaf and internal parts of the
    // octree that we wont use during searches
    LeafPart leaf;
    InternalPart inter;
    ...
    if (log) {
      log->buildTime =
          log->linearOctreeLeafTime + log->linearOctreeInternalTime;
      log->maxLeafPoints = maxPointsLeaf_;
      ...
      log->minRadiusAtMaxDepth =
          std::min(precomputedRadii[maxDepthSeen].getX(),
                   std::min(precomputedRadii[maxDepthSeen].getY(),
...
public:
  /**
   * @brief Builds the linear octree given an array of points, also reporting
   * how much time each step takes
   *
   * @details The points will be sorted in-place by the order given by the
   * encoding to allow spatial data locality
   */
  explicit LinearOctree(Container &points, std::vector<key_t> &codes, Box box,
                        PointEncoder &enc, const size_t maxPointsLeaf = 128,
                        std::shared_ptr<BuildLog> log = nullptr)
      : points(points), enc(enc), codes(codes), box(box),
        maxPointsLeaf_(maxPointsLeaf) {
    static_assert(
        !std::is_same_v<std::decay_t<PointEncoder>, PointEncoding::NoEncoding>,
        "Encoder cannot be an instance of NoEncoding when using LinearOctree.");
    buildOctree(log);
  }

  /// @brief Returns the memory overhead of the octree (points cloud or codes
  /// are not counted)
  size_t computeMemoryFootprint() const {
    size_t memory = 0;
    // Base size of the structure
    memory += sizeof(LinearOctree);
    // Size of each of the 4 arrays used in neighbourhood searches (the others
    // are deleted after build)
  ...
  /// @brief Computes the density of the point cloud as number of points /
  /// dataset
  double getDensity() const {
    return (double)points.size() / (box.radii().getX() * box.radii().getY() *
                                    box.radii().getZ() * 8.0f);
  }

  /// @brief Returns a const reference to the offsets array (for testing and
  /// analysis)
  const std::vector<uint32_t> &getOffsets() const { return offsets; }

  /// @brief Returns a const reference to the internal ranges array (for testing
  /// and analysis)
  const std::vector<std::pair<size_t, size_t>> &getInternalRanges() const {
    return internalRanges;
  }

  /// @brief Returns a const reference to the centers array (for testing and
  /// analysis)
  const std::vector<Point> &getCenters() const { return centers; }

  /// @brief Returns a const reference to the precomputed radii array (for
  ...
  size_t getMaxPointsLeaf() const { return maxPointsLeaf_; }

  /**
   * @brief Helper function to traverse the octree in a single pass
   *
   * @details This function is used to traverse the octree in a single pass,
   * calling the continuationCriterion function to decide whether to descend
   * into a node or not, and the endpointAction function to perform an action
   * when a leaf node is reached.
   *
   * @param continuationCriterion A function that takes the index of an internal
   * node indicates when to prune the tree during the search
   * @param endpointAction A function that takes the index of a leaf node and
   * computes an action over it
   */
  ...
  /**
   * @brief Search neighbors function. Similar to neighborsPrune(), but returns
   * a list-of-ranges wrapper structure containing the neighbours. This
   * structure implements a forward iterator and can thus be used in a loop. It
   * is way faster as it does not need to copy pointers to each individual point
   * found.
   * @param k specific kernel that contains the data of the region (center and
   * radius)
  ...
  [[nodiscard]] NeighborSet<Container> neighborsStruct(const Kernel &k) const {
    ...
    auto checkBoxIntersect = [&](uint32_t nodeIndex, uint32_t currDepth) {
      ...
      case KernelAbstract::IntersectionJudgement::INSIDE: {
        ...
        result.addRange(internalRanges[nodeIndex].first,
                        internalRanges[nodeIndex].second);
        return false;
      }
  ...
  /**
   * @brief Search neighbors function. Given kernel that already contains a
   * point and a radius, return the points inside the region. Note that
   * neighborsStruct is generally faster, so only use this function if you don't
   * want your result encapsultated on NeighborSet.
   *
   * @param k specific kernel that contains the data of the region (center and
   * radius)
   * @return Points inside the given kernel type.
  ...
  /**
   * @brief Search neighbors function. Basic implementation without "octant
   * contained in kernel" check. Note that this implementation is slower than
   * all the others, only use if you wanna apply an arbitrary condition
   * predicate apart from the kernel. neighborsPrune could also be adapted to
   * take an arbitrary predicate if needed.
   *
   * @param k specific kernel that contains the data of the region (center and
   * radius)
   * @param condition function that takes a candidate neighbor point and imposes
   * an additional condition (should return a boolean). The signature of the
   * function should be equivalent to `bool cnd(const typename &p);`
   * @return Points inside the given kernel type and satisfying condition.
   */
  template <typename Kernel, typename Function>
  [[nodiscard]] std::vector<size_t> neighbors(const Kernel &k,
                                              Function &&condition) const {
    std::vector<size_t> ptsInside;

    auto intersectsKernel = [&](uint32_t nodeIndex, uint32_t nodeDepth) {
      return k.boxOverlap(this->centers[nodeIndex],
                          this->precomputedRadii[nodeDepth]);
    };
  ...
  struct OctantPointIndex {
    ...
    bool operator<(const OctantPointIndex &other) const {
      // Note: std::priority_queue uses max heap by default, so invert
      // comparison for min-heap behavior
  ...
  // Helper for kNN searches
  inline static double distPointOctantSquared(const Point &p,
                                              const Point &octCenter,
                                              const Vector &octRadius) {
  ...
  /**
   *
   * @brief kNN search function. A priority queue with distances to center,
   * ascending order, both octants and points can be inside it.
   *
   * @details For octants, store the cube-to-point distance
   *  -> this is computed by observing that the closest point to it is
   * clamp(p_c, c_min, c_max) (for each coordinate c \in {x,y,z}) for points,
   * store the regular point-to-point distance
   *
   * algo:
   ...
   *  3.2 if it is an internal node, push the 8 suboctants with their min
   * distances to p
   *
   ...
   */
  size_t knn(const Point &p, const size_t k, std::vector<size_t> &indexes,
      ...
      if (top.isOctant) {
        if (offsets[top.index] == 0) {
          ...
          for (int octant = 0; octant < OCTANTS_PER_NODE; ++octant) {
            ...
            double sqDist = distPointOctantSquared(
                p, centers[childOctIndex], precomputedRadii[childDepth]);
  ...
  uint32_t getPrecisionLevel(const Vector &toleranceVector) const {
    assert(toleranceVector.getX() > 0 && toleranceVector.getY() > 0 &&
           toleranceVector.getZ() > 0 &&
           "tolerance vector must be >0 in every coordinate");
    uint32_t precisionLevel = maxDepthSeen;
    for (uint32_t i = 0; i < maxDepthSeen; i++) {
      Vector radii = precomputedRadii[i];
      if (radii.getX() < toleranceVector.getX() &&
          radii.getY() < toleranceVector.getY() && radii.getZ() &&
          toleranceVector.getZ()) {
        precisionLevel = i;
        break;
      }
    }
  ...
  // Helper for approx searches
  uint32_t getPrecisionLevel(const Vector &kernelRadii,
                             double tolerancePercentage) const {
    assert(tolerancePercentage > 0.0 &&
           "tolerance percentage must be greater than 0");
    return getPrecisionLevel(
        Vector{kernelRadii.getX() * tolerancePercentage / 100.0,
               kernelRadii.getY() * tolerancePercentage / 100.0,
               kernelRadii.getZ() * tolerancePercentage / 100.0});
  }

  /**
   * @brief Search neighbors function. Same algorithm as neighborsStruct(), but
   * the search is cut until a certain depth is reached.
   * @param[in] precisionLevel The maximum depth of the search
   * @param[in] Whether to include the points found an an intersecting leaf on
   * the maximum level, or not.
   * @return Points inside the given kernel type.
   */
  template <typename Kernel>
  [[nodiscard]] NeighborSet<Container> neighborsApprox(const Kernel &k,
                                                       uint32_t precisionLevel,
    ...
    auto checkBoxIntersect = [&](uint32_t nodeIndex, uint32_t currDepth) {
      ...
      if (currDepth > precisionLevel) {
        // If we are beyond precision and in upper bound mode, add octant to the
        // result
        if (upperBound) {
          result.addRange(internalRanges[nodeIndex].first,
                          internalRanges[nodeIndex].second);
  ...
  // Overrides for working with a given radius/vector of radii before calling
  // the actual search methods
  template <Kernel_t kernel_type = Kernel_t::square>
  [[nodiscard]] inline NeighborSet<Container>
  neighborsStruct(const Point &p, double radius) const {
  ...
  [[nodiscard]] inline NeighborSet<Container>
  neighborsStruct(const Point &p, const Vector &radii) const {
    const auto kernel = kernelFactory<kernel_type>(p, radii);
    return neighborsStruct(kernel);
  }

  template <Kernel_t kernel_type = Kernel_t::square>
  [[nodiscard]] inline std::vector<size_t> neighborsPrune(const Point &p,
                                                          double radius) const {
    const auto kernel = kernelFactory<kernel_type>(p, radius);
  ...
  neighborsPrune(const Point &p, const Vector &radii) const {
    const auto kernel = kernelFactory<kernel_type>(p, radii);
    return neighborsPrune(kernel);
  }

  template <Kernel_t kernel_type = Kernel_t::square>
  [[nodiscard]] inline std::vector<size_t> neighbors(const Point &p,
                                                     double radius) const {
    const auto kernel = kernelFactory<kernel_type>(p, radius);
  ...
  template <Kernel_t kernel_type = Kernel_t::cube>
  [[nodiscard]] inline std::vector<size_t>
  neighbors(const Point &p, const Vector &radii) const {
    const auto kernel = kernelFactory<kernel_type>(p, radii);
    constexpr auto dummyCondition = [](const Point &) { return true; };
    return neighbors(kernel, dummyCondition);
  }
  ...
  [[nodiscard]] inline std::vector<size_t>
  neighbors(const Point &p, double radius, Function &&condition) const {
    const auto kernel = kernelFactory<kernel_type>(p, radius);
  ...
  template <Kernel_t kernel_type = Kernel_t::square, class Function>
  [[nodiscard]] inline std::vector<size_t>
  neighbors(const Point &p, const Vector &radii, Function &&condition) const {
    const auto kernel = kernelFactory<kernel_type>(p, radii);
    return neighbors(kernel, std::forward<Function &&>(condition));
  }
  ...
  template <Kernel_t kernel_type = Kernel_t::square>
  [[nodiscard]] inline NeighborSet<Container>
  neighborsApprox(const Point &p, double radius, double tolerancePercentage,
                  bool upperBound) const {
    const auto kernel = kernelFactory<kernel_type>(p, radius);
  ...
  template <Kernel_t kernel_type = Kernel_t::cube>
  [[nodiscard]] inline NeighborSet<Container>
  neighborsApprox(const Point &p, const Vector &radii,
                  double tolerancePercentage, bool upperBound) const {
    const auto kernel = kernelFactory<kernel_type>(p, radii);
    return neighborsApprox(kernel, tolerancePercentage, upperBound);
  }
  ...
  template <Kernel_t kernel_type = Kernel_t::square>
  [[nodiscard]] inline NeighborSet<Container>
  neighborsApprox(const Point &p, double radius, const Vector &toleranceVector,
                  bool upperBound) const {
    const auto kernel = kernelFactory<kernel_type>(p, radius);
  ...
  [[nodiscard]] inline NeighborSet<Container>
  neighborsApprox(const Point &p, const Vector &radii,
                  const Vector &toleranceVector, bool upperBound) const {
    const auto kernel = kernelFactory<kernel_type>(p, radii);
    return neighborsApprox(kernel, toleranceVector, upperBound);
  }
  ...
  void writeVector(std::ofstream &file, std::vector<Time_t> &v,
                   std::string name = "v") {
  ...
  void writeVectorPairs(std::ofstream &file, std::vector<std::pair<U, V>> &v,
                        std::string name = "v") {
    file << "Printing vector " << name << " with " << v.size() << "elements\n";
    for (size_t i = 0; i < v.size(); i++)
  ...
  template <typename Time_t>
  void writeVectorBinary(std::ofstream &file, std::vector<Time_t> &v,
                         std::string name = "v") {
    file << "Printing vector " << name << " with " << v.size() << "elements\n";
    for (size_t i = 0; i < v.size(); i++)
  ...
  void writePointsAndCodes(std::ofstream &file,
                           const std::string &encoder_name) {
    file << std::fixed << std::setprecision(3);
    file << encoder_name << " " << "x y z\n";
    assert(codes.size() == points.size());
    for (size_t i = 0; i < codes.size(); i++)
      file << codes[i] << " " << points[i].getX() << " " << points[i].getY()
           << " " << points[i].getZ() << "\n";
  }

  void logOctree(std::ofstream &file, std::ofstream &pointsFile, LeafPart &leaf,
                 InternalPart &inter) {
    ...
    file << "Max. points per leaf: " << maxPointsLeaf_ << "\n";
  ...
  void logOctreeBounds(std::ofstream &outputFile, int max_level) {
    ...
    auto logBounds = [&](uint32_t nodeIndex, uint32_t currDepth) {
      ...
      outputFile << currDepth << "," << up.getX() << "," << up.getY() << ","
                 << up.getZ() << "," << down.getX() << "," << down.getY() << ","
                 << down.getZ() << "\n";
